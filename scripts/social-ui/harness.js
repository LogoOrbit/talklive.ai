...
async function start({ port }) {
  ...
  const srv = spawn(process.execPath, [REPO + '/server/index.js'], {
    env: { ...process.env, PORT: String(port), DATA_DIR: dir, NODE_ENV: 'development', GIPHY_API_KEY: '', CALL_BACK_LIVE_MS: '500' },
...
function bot(base, name) {
  ...
  s.emit('register', { clientId: ID[name], nickname: name[0].toUpperCase() + name.slice(1), gender: 'male' });
  return s;
}
...
async function browser(base, { path: p = '/', clientId = ID.ann, lang = 'en', init = null } = {}) {
  ...
  await ctx.addInitScript(([cid, lang]) => {
    try {
      if (!localStorage.getItem('talklive_client_id')) localStorage.setItem('talklive_client_id', cid);
...
module.exports = { start, bot, browser, wait, ok, once, ID, failed: () => failed };
