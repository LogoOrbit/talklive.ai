...
(async () => {
  ...
  await wait(500);
  await click('#friendChatBlockBtn'); await wait(200);
  const warn = await q(() => { const m = document.querySelector('.modal-overlay:not(.hidden) [role="alertdialog"]'); return m ? m.innerText.replace(/\s+/g, ' ') : ''; });
  s = await snap();
  ok('block asks first, with a warning', s.chatOpen && /Block Dan\?/.test(warn) && /won't be matched|not be matched/.test(warn), warn);
  await click('.modal-overlay:not(.hidden) .btn-secondary'); await wait(200);
  s = await snap();
  ok('cancel keeps the chat and blocks nothing', s.chatOpen && !(await q(() => document.querySelector('.modal-overlay:not(.hidden) [role="alertdialog"]'))));
  await click('#friendChatBlockBtn'); await wait(200);
  await click('.modal-overlay:not(.hidden) .btn-danger'); await wait(600);
  s = await snap();
  ok('confirming blocks and closes', !s.chatOpen && /Blocked/.test(s.toast), s.toast);
