...
(async () => {
  ...
  try {
    ...
    ok('account gets a name#1234 ID it can edit', aSigned.id.temporary === false && aSigned.id.editable === true && /^[a-z]{3,16}#\d{4}$/.test(accountId), JSON.stringify(aSigned.id));
    ...
    const chosen = await setId(aAfter.sock, 'AlphaWolf#0042');
    ok('an account can choose its ID (stored lowercase)', chosen.ok && chosen.friendId === 'alphawolf#0042', JSON.stringify(chosen));
    ok('the new ID finds them', (await search(c.sock, 'ALPHAWOLF#0042')).ok);
    ...
    ok('numbers or symbols in the name are refused', (await setId(aAfter.sock, 'alpha_1#0042')).ok === false && (await setId(aAfter.sock, 'al.pha#0042')).ok === false);
    ok('letters in the tag are refused', (await setId(aAfter.sock, 'alpha#00a2')).ok === false);
    ...
    const clash = await setId(cSigned.sock, 'alphawolf#0042');
    ...
    const nameOnly = await setId(cSigned.sock, 'alphawolf');
    ok('a name alone gets a free number', nameOnly.ok && /^alphawolf#\d{4}$/.test(nameOnly.friendId) && nameOnly.friendId !== 'alphawolf#0042', JSON.stringify(nameOnly));
    const foundChosen = await search(cSigned.sock, 'alphawolf#0042');
    ok('search results show the chosen ID', foundChosen.ok && foundChosen.user.friendId === 'alphawolf#0042', JSON.stringify(foundChosen));
